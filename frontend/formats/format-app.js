/**
 * FORMAT-APP.JS - V10.3 FINAL CONSOLIDATED
 * - Soporte Completo: FO-LC-17 a FO-LC-45 (Todos los módulos).
 * - FO-LC-24: Actualizado con notación científica y nuevos productos.
 * - FO-LC-21: Mantenida lógica de subcultivos y bitácora.
 * - FO-LC-20: Mantenida lógica de generación de códigos.
 * - Generics: Mantenido soporte para formatos nuevos (40-45).
 */
...
const App = {
    ...
    init: function() {
        ...
        console.log("Iniciando App V10.3 para:", docId);
        ...
        // Enrutador de lógica según el ID del documento
        switch(docId) {
            // Documentos simples (solo carga/guardado genérico ya manejado por Universal)
            case 'doc-fo-lc-ResumenRecepcion': 
            case 'doc-fo-lc-17': 
            case 'doc-fo-lc-18': 
            case 'doc-fo-lc-19': 
            case 'doc-fo-lc-23': 
            case 'doc-fo-lc-28': // A veces usa ID 40, manejado abajo si es el caso
                break;
            
            // Documentos con lógica específica
            ...
            // Documentos nuevos / genéricos (Tablas dinámicas)
            case 'doc-fo-lc-40': // Cubre Prep Medios
            case 'doc-fo-lc-41': // Micro
            case 'doc-fo-lc-42': // Liof Placenta
            case 'doc-fo-lc-43': // Liof MC
            case 'doc-fo-lc-44': // Lib Flasks
            case 'doc-fo-lc-45': // Embalaje
                this.Docs.FO_Generic.init(docId); 
                break;
        }
    },
    ...
    Universal: {
        ...
        saveData: function() {
            ...
            // Guardar datos custom de los módulos específicos
    ...
    Docs: {
                ...
                if (action === 'alim') {
                    Swal.fire({ title: `Alim: ${codigo}`, input: 'text', showCancelButton: true }).then(res => {
                        if (res.value) {
                            ...
                            row.cells[5].querySelector('input').value = today; 
        ...
        // ==========================================
        // FO-LC-24: DOSIS (UPDATED WITH SCI NOTATION)
        // ==========================================
        FO_LC_24: {
            productsDB: { 
            ...
            init: function() { 
                ...
                this.calcInventory(); 
                document.querySelector('#tbl-dosis tbody').addEventListener('input', (e) => {
                    if(e.target.classList.contains('cell-input') || e.target.classList.contains('prod-select') || e.target.classList.contains('obs-input')) {
                        this.calcInventory();
                    }
                });
            },
            
            // Helper: 2.7e+9 (Científica para parciales)
            formatScientific: function(num) {
                if (num === 0 || isNaN(num)) return "0";
                return num.toExponential(2).replace('+', '');
            },
            
            // Helper: 2.5G (Sufijos para Grand Total)
            formatSuffix: function(num) {
                if (num === 0 || isNaN(num)) return "0";
            ...
            addDosis: function() {
                ...
                row.innerHTML = `<td class="text-center row-num">${n}</td>
                    <td><select class="cedit prod-select" onchange="App.Docs.FO_LC_24.onProd(this)"><option value="">Sel...</option><option value="Stem Xelle">Stem Xelle</option><option value="Hybrid Xelle">Hybrid Xelle</option><option value="Stem Ortho">Stem Ortho</option><option value="Hybrid Ortho">Hybrid Ortho</option><option value="X-Exosomes">X-Exosomes</option></select></td>
                    ...
                    <td><input class="cedit cell-input" type="number" step="any" placeholder="Cél."></td>
            ...
            onProd: function(s) {
                ...
                const c=this.productsDB[s.value]; 
                const p=r.querySelector('.pres-select'); 
                p.innerHTML=''; 
            ...
            updateCode: function(r) {
                const l=r.querySelector('.lote-input').value; 
                const n=r.querySelector('.row-num').innerText.padStart(3,'0');
            ...
            calcInventory: function() {
                const vials = { "Stem Xelle":0, "Hybrid Xelle":0, "Stem Ortho":0, "Hybrid Ortho":0, "X-Exosomes":0 };
                const cells = { "Stem Xelle":0, "Hybrid Xelle":0, "Stem Ortho":0, "Hybrid Ortho":0, "X-Exosomes":0 };
                ...
                document.querySelectorAll('#tbl-dosis tbody tr').forEach(r => {
                    ...
                    if(!obs) { 
                        ...
                        const cVal = parseFloat(r.querySelector('.cell-input').value) || 0;
                        
                        if(vials[p] !== undefined) {
                            vials[p]++;
                            cells[p] += cVal;
                        }
                    }
                ...
                // Update Viales (integer)
                document.getElementById('tot-stem').value = vials["Stem Xelle"];
                document.getElementById('tot-hybrid').value = vials["Hybrid Xelle"];
                document.getElementById('tot-stem-ortho').value = vials["Stem Ortho"];
                document.getElementById('tot-hybrid-ortho').value = vials["Hybrid Ortho"];
                document.getElementById('tot-exo').value = vials["X-Exosomes"];
                
                // Update Celulas (Scientific Notation for partials)
                document.getElementById('cell-stem').value = this.formatScientific(cells["Stem Xelle"]);
                document.getElementById('cell-hybrid').value = this.formatScientific(cells["Hybrid Xelle"]);
                document.getElementById('cell-stem-ortho').value = this.formatScientific(cells["Stem Ortho"]);
                document.getElementById('cell-hybrid-ortho').value = this.formatScientific(cells["Hybrid Ortho"]);
                document.getElementById('cell-exo').value = this.formatScientific(cells["X-Exosomes"]);

                // Grand Totals
                const totalVials = Object.values(vials).reduce((a,b)=>a+b, 0);
                const totalCells = Object.values(cells).reduce((a,b)=>a+b, 0);
                
                document.getElementById('grand-tot-vials').value = totalVials;
                // Suffix notation for Grand Total
                document.getElementById('grand-tot-cells').value = this.formatSuffix(totalCells);
            },
            
            getCustomData: function() {
                const r=[]; 
                document.querySelectorAll('#tbl-dosis tbody tr').forEach(tr=>{ 
                    const i=tr.querySelectorAll('input,select,textarea'); 
                    if(i[0].value) r.push({ 
                        p:i[0].value, o:i[1].value, c:i[2].value, l:i[3].value, 
                        pr:i[4].value, cel:i[5].value, v:i[6].value, u:i[7].value, ob:i[8].value 
                    }); 
                });
            ...
            loadCustomData: function(d) {
                ...
                if(d.t_doses) {
                    ...
                    d.t_doses.forEach(data=>{
                        ...
                        const i=r.querySelectorAll('input,select,textarea');
                        
                        i[0].value=data.p; this.onProd(i[0]); 
                        i[1].value=data.o; App.Universal.autoResize(i[1]); 
                        i[2].value=data.c; 
                        i[3].value=data.l; 
                        i[4].value=data.pr; 
                        i[5].value=data.cel; 
        ...
        // ==========================================
        // GENERIC MODULE (For docs 40-45 & others)
        // ==========================================
        FO_Generic: { 
            init: function(id) { 
                // Auto-add first row if empty
                if(document.querySelector('table tbody') && document.querySelector('table tbody').children.length === 0) { 
                    if(id.includes('41')) window.addMuestra41(); 
                    else if(id.includes('42')) window.addGenericRow('tbl-mp', `<td><input class='cedit'></td><td><input type='date' class='cedit'></td><td><input class='cedit'></td><td><input type='number' class='cedit'></td><td class='no-print'><button class='btn-danger btn-mini' onclick='this.closest("tr").remove()'>x</button></td>`); 
                } 
            } 
        },
        // Helper interno para el generic
        addGenericRow: function(id, html) { 
            const tbody = document.querySelector(`#${id} tbody`);
            if(tbody) {
                const r=document.createElement('tr'); r.innerHTML=html; tbody.appendChild(r); 
            }
        }
    }
};
...
// Mapeo de funciones globales para uso en HTML (onclick)
window.saveForm = () => App.Universal.saveData(); 
window.printForm = () => App.Universal.printForm(); 
window.clearForm = () => App.Universal.clearForm(); 

// Helpers genéricos de filas
window.addGenericRow = (i,h) => App.Docs.addGenericRow(i,h);

// Helpers específicos de FO-LC-20
window.addFlaskRow = () => App.Docs.FO_LC_20.addFlaskRow(); 
window.addSupplyRow = () => {
    const r=document.createElement('tr');
    r.innerHTML=`<td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="date"></td><td class="no-print"><button class="btn btn-danger btn-mini" onclick="this.closest('tr').remove()">X</button></td>`;
    document.getElementById('supplies-table-body').appendChild(r);
}; 
window.addFreezeRow = () => {
    const r=document.createElement('tr');
    r.innerHTML=`<td><input type="number" style="width:50px" value="1"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text"></td><td><input type="text" value="DMSO 10%"></td><td><input type="text"></td><td><input type="text"></td><td><select><option>Vial</option><option>Bolsa</option></select></td><td><input type="text"></td><td class="no-print"><button class="btn btn-danger btn-mini" onclick="this.closest('tr').remove()">X</button></td>`;
    document.getElementById('freeze-table-body').appendChild(r);
};

// Helpers específicos de FO-LC-21
window.agregarFilaInsumo = () => {
    const r=document.createElement('tr');
    r.innerHTML=`<td><input class="cedit"></td><td><input class="cedit"></td><td><input class="cedit"></td><td><input type="date" class="cedit"></td><td class="no-print"><button class="btn-danger btn-mini" onclick="this.closest('tr').remove()">X</button></td>`;
    document.querySelector('#tabla-insumos-dia tbody').appendChild(r);
}; 
window.addFlaskManual21 = () => App.Docs.FO_LC_21.addFlaskManual();

// Helpers específicos de FO-LC-24
window.addDosis24 = () => App.Docs.FO_LC_24.addDosis(); 

// Helpers específicos de FO-LC-41
window.addMuestra41 = () => App.Docs.addGenericRow('tbl-micro', `<td><input class='cedit'></td><td><input type='date' class='cedit'></td><td><select class='cedit'><option>-</option><option>NEG</option><option>POS</option><option>NA</option></select></td><td><select class='cedit'><option>-</option><option>NEG</option><option>POS</option></select></td><td><select class='cedit'><option>-</option><option>NEG</option><option>POS</option></select></td><td><input type='date' class='cedit'></td><td><input class='cedit'></td><td class='no-print'><button class='btn-danger btn-mini' onclick='this.closest("tr").remove()'>x</button></td>`);